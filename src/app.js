...
let state = { tab: 'import', pasteText: '', rows: loadRows(), errors: [], selectedTicker: null, search: '' };
...
function renderTopbar() {
  ...
  return `<header class="topbar"><div class="topbar-inner"><div class="brand"><div class="brand-mark">↗</div><span>Stock Mind</span></div><nav class="tabs">
...
function renderList() {
  ...
  return `<section class="panel panel-pad"><div class="toolbar"><div><p class="eyebrow">Ranking</p><h2>${rows.length} mã</h2><p class="muted">Bấm vào một mã để tự động chuyển sang CRSM Screened.</p></div><input class="search" id="searchInput" value="${escapeHtml(state.search)}" placeholder="Tìm ticker hoặc ngành"></div>
    <div class="table-wrap"><table><thead><tr><th>Rank</th><th>Ticker</th><th>Industry</th><th>Price</th><th>P/E</th><th>Quality</th><th>Growth</th><th>Value</th><th>Micro</th><th>Momentum</th><th>Mispricing</th><th>Final</th><th>Grade</th></tr></thead>
    <tbody>${rows.map(row => `<tr class="clickable" data-crsm-row="${row.TICKER}" title="Mở CRSM Screened cho ${row.TICKER}"><td>#${row.RANK}</td><td class="ticker">${row.TICKER}<button class="crsm-link" data-crsm="${row.TICKER}" title="Phân tích bằng CRSM">CRSM →</button><button class="crsm-link" data-detail="${row.TICKER}" title="Xem chi tiết scoring">Chi tiết</button></td><td>${escapeHtml(row.INDUSTRY)}</td><td>${fmt(row.PRICE)}</td><td>${fmt(row.PE)}</td><td>${fmt(row.QUALITY_SCORE)}</td><td>${fmt(row.GROWTH_SCORE)}</td><td>${fmt(row.VALUATION_SCORE)}</td><td>${fmt(row.MICRO)}</td><td>${fmt(row.MOMENTUM)}</td><td>${fmt(row.MISPRICING)}</td><td><strong>${fmt(row.FINALSCORE)}</strong></td><td>${gradeBadge(row.GRADE)}</td></tr>`).join('')}</tbody></table></div></section>`;
}
...
function bindEvents() {
  ...
  document.querySelectorAll('[data-crsm-row]').forEach(row => row.addEventListener('click', () => launchScreenedCRSM(row.dataset.crsmRow)));
  ...
  bind('clearData', 'click', () => { state.rows = []; state.errors = []; state.selectedTicker = null; localStorage.removeItem(STORAGE_KEY); render(); });
...
function stockLine(row) { return `<div class="stock-line clickable" data-crsm-row="${row.TICKER}" title="Mở CRSM Screened cho ${row.TICKER}"><span><strong>${row.TICKER}</strong> <span class="muted">${escapeHtml(row.INDUSTRY)}</span></span><span>${fmt(row.FINALSCORE)} ${gradeBadge(row.GRADE)}</span></div>`; }
