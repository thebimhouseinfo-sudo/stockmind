...
const LEGACY_ALIASES = {
  ...
  REVGROWTH: 'revenue_growth_annual_yoy', EPSGROWTH: 'eps_dil_growth_ttm_yoy',
  DEBT: 'debt_equity_fq', PE: 'pe', PEG: 'peg', RET1M: 'perf_1m', RET3M: 'perf_3m',
  RET6M: 'perf_6m', RET12M: 'perf_1y'
};

const HEADER_ALIASES = {
  ticker: ['symbol', 'ticker', 'ma', 'ma ck'], company_name: ['company', 'company name', 'name', 'ten cong ty'],
  sector: ['sector'], industry: ['industry', 'nganh'], market_cap: ['mkt cap', 'market cap'], price: ['price', 'last', 'close', 'gia'],
  change_pct: ['chg', 'chg %', 'change', 'change %'], perf_1w: ['perf 1w', 'performance 1w'], perf_1m: ['perf 1m', 'performance 1m'],
  perf_3m: ['perf 3m', 'performance 3m'], perf_6m: ['perf 6m', 'performance 6m'], perf_1y: ['perf 1y', 'performance 1y'], perf_ytd: ['perf ytd', 'performance ytd'],
  ...
  revenue_growth_quarterly_yoy: ['revenue growth quarterly yoy'], revenue_growth_annual_yoy: ['revenue growth annual yoy', 'revenue growth yoy'],
  eps_dil_ttm: ['eps dil ttm', 'eps ttm'], eps_dil_growth_ttm_yoy: ['eps dil growth ttm yoy', 'eps growth ttm yoy'], peg_ttm: ['peg ttm'],
  gross_margin_ttm: ['gross margin % ttm', 'gross margin ttm'], operating_margin_ttm: ['op margin % ttm', 'operating margin % ttm', 'operating margin ttm'],
  net_margin_ttm: ['net margin % ttm', 'net margin ttm'], fcf_ttm: ['fcf ttm', 'free cash flow ttm'], fcf_growth_ttm_yoy: ['fcf growth ttm yoy'],
  debt_equity_fq: ['debt/equity fq', 'debt equity fq'], debt_equity_fy: ['debt/equity fy', 'debt equity fy'], current_ratio_fq: ['current ratio fq'], current_ratio_fy: ['current ratio fy'],
  quick_ratio_fq: ['quick ratio fq'], quick_ratio_fy: ['quick ratio fy'], pe: ['p/e', 'pe', 'price to earnings'], peg: ['peg'], pb: ['p/b', 'pb'], ps: ['p/s', 'ps'],
  ev_ebitda: ['ev/ebitda', 'ev ebitda'], ev_revenue: ['ev/revenue', 'ev revenue'], dividend_yield_ttm: ['div yield % ttm', 'dividend yield % ttm', 'dividend yield ttm']
};
...
function parseTradingViewFourLine(lines) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    for (let index = 0; index < TRADINGVIEW_COLUMNS.length; index += 1) {
      ...
      row[field] = field === 'sector' || field === 'industry' ? cleanText(value) : (PERCENT_FIELDS.has(field) ? cleanPercent(value) : cleanNumber(value));
    }
    if (!row.industry) row.industry = 'Unknown';
    applyLegacyAliases(row); rows.push(row); i += 3;
  }
...
function normalizeHeaderRow(cells, columns, sourceRow) { const row = { sourceRow }; for (const field of Object.keys(HEADER_ALIASES)) { const index = columns[field]; const raw = index == null ? null : cells[index]; if (field === 'ticker') row.ticker = extractPlainTicker(raw) || cleanText(raw); else if (field === 'company_name' || field === 'sector' || field === 'industry') row[field] = cleanText(raw); else row[field] = PERCENT_FIELDS.has(field) ? cleanPercent(raw) : cleanNumber(raw); } if (!row.industry) row.industry = 'Unknown'; applyLegacyAliases(row); return row; }
...
export function cleanNumber(value) {
  ...
  text = text.replace(/\u2212/g, '-').replace(/−/g, '-').replace(/\u202f/g, '').replace(/\u00a0/g, '').replace(/\s/g, '').replace(/^\+/, '').replace(/[KMBT]$/i, '').replace(/,/g, '').replace('%', '');
  const number = Number.parseFloat(text); if (!Number.isFinite(number)) return null; return number * multiplier;
}
...
function cleanPercent(value) {
  ...
  text = text.replace(/\u2212/g, '-').replace(/−/g, '-').replace(/\u202f/g, '').replace(/\u00a0/g, '').replace(/\s/g, '').replace(/^\+/, '').replace('%', '').replace(/,/g, '');
  const number = Number.parseFloat(text); if (!Number.isFinite(number)) return null;
  // Internal percentage fields are stored in percentage points, not ratios:
  // -0.58% -> -0.58, 8% -> 8. If TradingView omits the % sign and provides a ratio
  // such as 0.08, convert it to 8 percentage points. Explicit percent text is never /100.
  return hasPercent ? number : (Math.abs(number) <= 1 ? number * 100 : number);
}
