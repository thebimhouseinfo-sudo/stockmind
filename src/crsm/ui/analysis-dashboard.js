        ...
        <aside class="crsm-left-rail">
          ...
          <section class="crsm-side-card crsm-cost-card"><div class="crsm-side-title">CHI PHÍ</div>
            <div class="crsm-node-cost-table">
              ...
              ${nodeCostRows.map(row => `<div class="crsm-node-cost-row ${row.className || ''}"><span title="${escapeHtml(row.label)}">${escapeHtml(row.shortLabel)}</span><b>${row.tokens.toLocaleString('vi-VN')}</b><strong>$${row.cost.toFixed(4)}</strong></div>`).join('')}
            </div>
...
function buildNodeCostRows() {
  ...
  return ['node1', 'node2', 'node3', 'node4', 'node5', 'node6', 'node7'].map(nodeId => {
    const row = byNode.get(nodeId) || {};
    return {
      nodeId,
      label: NODE_LABELS[nodeId] || nodeId,
      shortLabel: nodeId === 'node6' ? 'NODE6 (6A+6B)' : nodeId.toUpperCase(),
      className: nodeId === 'node6' ? 'report' : nodeId === 'node7' ? 'local' : '',
