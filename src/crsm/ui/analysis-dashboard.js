...
export function renderAnalysisDashboard() {
  ...
  const pendingCount = Math.max(0, total - completedCount - running);
  ...
  const activities = (crsmState.logRows || []).slice(-10).reverse();
      ...
      <section class="crsm-main-grid">
        <aside class="crsm-left-rail">
          ...
          <section class="crsm-side-card">
            ...
            ${failedCount ? `<div class="crsm-legend-row"><span class="dot red"></span><span>Có lỗi</span><b>${failedCount}</b></div>` : ''}
          </section>

          <section class="crsm-side-card crsm-cost-card">
            <div class="crsm-side-title">CHI PHÍ LẦN CHẠY</div>
            <div class="crsm-cost-grid">
              <div><span>Chi phí</span><strong>$${Number(usage.cost || 0).toFixed(4)}</strong></div>
              <div><span>Tokens</span><strong>${((usage.input || 0) + (usage.output || 0)).toLocaleString('vi-VN')}</strong></div>
              <div><span>Requests</span><strong>${crsmState.usage?.length || 0}</strong></div>
            </div>
          </section>

          <section class="crsm-side-card crsm-analysis-info">
...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    .crsm-hero{display:flex;justify-content:space-between;align-items:center;gap:20px;padding:22px 26px;border:1px solid #dbe5f2;border-radius:16px;background:linear-gradient(135deg,#ffffff 0%,#f6f9ff 100%);box-shadow:0 8px 28px rgba(37,99,235,.08);min-width:0}
    ...
    .crsm-run-card{display:flex;align-items:center;gap:18px;min-width:330px;padding:13px 16px;border:1px solid #dce6f2;border-radius:14px;background:#fff}.crsm-run-card small{display:block;color:#73839a;font-size:9px;font-weight:900;letter-spacing:.08em}.crsm-run-card strong{display:block;margin-top:5px;font-size:23px;color:#132341}.crsm-run-divider{width:1px;height:38px;background:#e2e8f0}.crsm-run-percent{display:flex;align-items:center;gap:9px;color:#2563eb;font-size:22px;font-weight:950}.crsm-mini-ring{width:28px;height:28px;border-radius:50%;display:inline-block;background:conic-gradient(#2563eb calc(var(--pct) * 1%),#e6edf6 0);position:relative}.crsm-mini-ring::after{content:'';position:absolute;inset:5px;border-radius:50%;background:#fff}
    .crsm-input-row{display:flex;align-items:center;gap:12px;padding:13px 16px;border:1px solid #dbe5f2;border-radius:14px;background:#fff;box-shadow:0 6px 18px rgba(18,35,65,.04);min-width:0}.crsm-input-title{font-size:10px;font-weight:900;color:#6f819a;white-space:nowrap}.crsm-input-slot{display:flex;align-items:center;gap:10px;flex:1;min-width:0}.crsm-dashboard-ticker{width:100%;max-width:320px;height:42px;border:1px solid #d5e0ef;border-radius:10px;padding:0 14px;background:#fbfdff;color:#132341;min-width:0}.crsm-dashboard-doc,.crsm-dashboard-run{height:42px;display:inline-flex;align-items:center;justify-content:center;border-radius:10px;padding:0 17px;font-weight:850;cursor:pointer;white-space:nowrap}.crsm-dashboard-doc{border:1px solid #d5e0ef;background:#fff;color:#193760}.crsm-dashboard-run{border:0;background:#2167eb;color:#fff;box-shadow:0 7px 16px rgba(33,103,235,.2)}.crsm-session-meta{display:flex;align-items:center;gap:9px;margin-left:auto;font-size:10px;color:#77889f;white-space:nowrap}.crsm-session-meta span{font-weight:900}.crsm-session-meta strong{padding:4px 8px;border-radius:999px;background:#efe7ff;color:#6346ff}.crsm-session-meta b{font-size:11px;color:#1e3557}
    .crsm-main-grid{display:grid;grid-template-columns:240px minmax(0,1fr) 300px;gap:14px;align-items:stretch;min-width:0}.crsm-left-rail,.crsm-right-rail{display:flex;flex-direction:column;gap:12px;min-width:0}.crsm-left-rail{height:100%}.crsm-side-card{padding:15px;border:1px solid #dbe5f2;border-radius:14px;background:#fff;box-shadow:0 6px 20px rgba(18,35,65,.04);min-width:0;overflow:hidden}.crsm-side-title{font-size:11px;letter-spacing:.05em;font-weight:950;color:#183e77}.crsm-progress-donut{width:122px;height:122px;margin:11px auto;border-radius:50%;display:grid;place-items:center;background:conic-gradient(#1f78ed calc(var(--pct)*1%),#e7edf5 0);position:relative}.crsm-progress-donut::after{content:'';position:absolute;inset:11px;border-radius:50%;background:#fff}.crsm-progress-donut>div{position:relative;z-index:1;text-align:center}.crsm-progress-donut strong{display:block;font-size:28px;color:#132341}.crsm-progress-donut span{font-size:10px;color:#64758d}.crsm-progress-row,.crsm-legend-row,.crsm-info-row{display:flex;align-items:center;justify-content:space-between;gap:8px;font-size:11px;padding:5px 0}.crsm-thin-bar{height:7px;border-radius:999px;background:#e8eef6;overflow:hidden;margin-top:3px}.crsm-thin-bar i{display:block;height:100%;background:#2f78eb;border-radius:999px}.crsm-progress-card>small{display:block;margin-top:4px;text-align:center;color:#65788e;font-size:10px}.dot{width:8px;height:8px;border-radius:50%;display:inline-block;margin-right:4px}.dot.green,.green{background:#1f9d65}.dot.blue,.blue{background:#2372ee}.dot.gray,.gray{background:#91a1b5}.dot.amber,.amber{background:#f4a61a}.dot.red,.red{background:#e35a5a}.crsm-legend-row span:nth-child(2){flex:1;color:#40536d}.crsm-legend-row b{color:#183e77}.crsm-info-row span{color:#697c95}.crsm-info-row b{color:#1c355a}
    .crsm-cost-card{padding-bottom:13px}.crsm-cost-grid{display:grid;grid-template-columns:1.2fr 1fr .8fr;gap:8px;margin-top:9px}.crsm-cost-grid>div{min-width:0;padding:8px;border:1px solid #edf1f6;border-radius:9px;background:#fbfdff}.crsm-cost-grid span{display:block;color:#7d8da1;font-size:8px;font-weight:800}.crsm-cost-grid strong{display:block;margin-top:4px;color:#183e77;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .crsm-pipeline-card{padding:19px 16px;border:1px solid #dbe5f2;border-radius:14px;background:#fff;box-shadow:0 6px 20px rgba(18,35,65,.04);min-width:0;overflow:hidden}.crsm-pipeline-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;min-width:0}.crsm-pipeline-head p{margin:5px 0 0;color:#71829a;font-size:11px}.crsm-status-key{display:flex;gap:12px;flex-wrap:wrap;justify-content:flex-end;font-size:10px;color:#65768e}.crsm-status-key span{display:flex;align-items:center;gap:4px}.crsm-status-key i{width:7px;height:7px;border-radius:50%;display:inline-block}
    .crsm-tree{position:relative;padding:11px 0 0;min-width:0}.crsm-tree-stage{position:relative;display:grid;grid-template-columns:minmax(0,1fr);gap:10px;padding:10px 0 16px;min-width:0}.crsm-tree-stage.is-parallel{grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.crsm-tree-stage.is-parallel:last-child{grid-template-columns:repeat(3,minmax(0,1fr))}.crsm-tree-stage:not(:first-child)::before{content:'';position:absolute;top:-10px;left:50%;width:2px;height:20px;background:#b8d2fa}.crsm-tree-stage.is-parallel::after{content:'';position:absolute;left:25%;right:25%;top:-10px;height:2px;background:#b8d2fa}.crsm-tree-node{position:relative;display:flex;align-items:center;gap:10px;min-height:78px;padding:13px 14px;border:1px solid #dbe6f4;border-radius:13px;background:#fff;box-shadow:0 3px 12px rgba(31,65,112,.04);transition:.2s;min-width:0;overflow:hidden}.crsm-tree-node::before{content:'';position:absolute;left:50%;top:-10px;width:2px;height:10px;background:#b8d2fa}.crsm-tree-stage:first-child .crsm-tree-node::before{display:none}.crsm-tree-icon{width:38px;height:38px;border-radius:12px;display:grid;place-items:center;background:#eef4fb;color:#2c6ee6;font-size:19px;font-weight:900;flex:0 0 auto}.crsm-tree-copy{display:flex;flex-direction:column;min-width:0;gap:4px;overflow:hidden}.crsm-tree-copy strong{font-size:13px;color:#153a74;white-space:normal;overflow-wrap:anywhere}.crsm-tree-copy span{font-size:10px;line-height:1.3;color:#6b7c93;white-space:normal;overflow-wrap:anywhere}.crsm-tree-state{margin-left:auto;font-size:19px;color:#91a1b5;flex:0 0 auto}.crsm-tree-node.running{border-color:#86b8ff;background:#f1f7ff;box-shadow:0 0 0 3px rgba(37,99,235,.07)}.crsm-tree-node.running .crsm-tree-icon{background:#2572eb;color:#fff;animation:crsmNodePulse 1s infinite}.crsm-tree-node.done{border-color:#bcead5;background:#f8fffb}.crsm-tree-node.done .crsm-tree-icon{background:#e8f8f0;color:#158a5a}.crsm-tree-node.done .crsm-tree-state{color:#169160}.crsm-tree-node.failed{border-color:#f2c3c3;background:#fff8f8}.crsm-tree-node.failed .crsm-tree-state{color:#cf4747}.crsm-tree-node em{position:absolute;right:10px;bottom:7px;font-size:9px;color:#d28a12;font-style:normal;font-weight:900}.spinner{display:block;width:18px;height:18px;border:2px solid #d8e5f5;border-top-color:#2a74ee;border-right-color:#2a74ee;border-radius:50%;animation:crsmSpin 1s linear infinite}
    .crsm-activity-list{display:flex;flex-direction:column;gap:9px;margin-top:11px;max-height:278px;overflow:auto;min-width:0;padding-right:3px}.crsm-activity-item{display:flex;gap:8px;align-items:flex-start;font-size:10px;line-height:1.35;color:#435772;min-width:0}.crsm-activity-item i{width:7px;height:7px;border-radius:50%;margin-top:4px;flex:0 0 auto}.crsm-activity-item span{min-width:0;overflow-wrap:anywhere;word-break:break-word}.crsm-inline-error-actions{display:flex;gap:8px;margin-top:11px;flex-wrap:wrap}.crsm-retry-btn{min-height:34px;padding:0 11px;border:0;border-radius:9px;background:#2167eb;color:#fff;font-size:10px;font-weight:850;cursor:pointer}.crsm-retry-btn.secondary{background:#edf2f8;color:#29496f;border:1px solid #d8e2ef}.crsm-secondary-btn{width:100%;margin-top:11px;height:34px;border:1px solid #dce5f1;border-radius:9px;background:#fff;color:#1f4f94;font-weight:800;font-size:10px;display:flex;align-items:center;justify-content:space-between;padding:0 12px}.crsm-session-box{padding:9px 0;border-bottom:1px solid #edf1f6}.crsm-session-box:last-child{border-bottom:0}.crsm-session-box span{display:block;font-size:9px;color:#7b8ca1;font-weight:900}.crsm-session-box strong{display:block;margin-top:4px;font-size:12px;color:#1a365d;overflow-wrap:anywhere}.crsm-empty{padding:10px 0;font-size:10px;color:#7b8ca1}
    #crsmDynamic>.crsm-cost-strip,#crsmDynamic>.crsm-error{display:none!important}
