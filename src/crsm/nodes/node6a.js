...
import { node6aPrompt } from '../prompts/node6a.js';

export async function node6a(ctx) {
  try {
    const { text } = await runLLM({
      nodeId: 'node6a',
      systemInstruction: buildNode6ASystemInstruction(),
      prompt: buildNode6APrompt(ctx),
      responseFormat: 'text'
    });
    const html = cleanHtml(text);
    if (isUsableHtml(html)) return html;
    console.warn('[node6a] LLM output was not usable HTML. Falling back to local renderer.');
  } catch (error) {
    console.warn('[node6a] LLM render failed. Falling back to local renderer.', error);
  }
  return renderNode6A(ctx);
}
...
export function renderNode6A(ctx) {
  ...
  const companyName = f('node1.company_name', ctx.ticker || n1.ticker || MISSING_SHORT);
...
<header class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4"><div><div class="flex items-center gap-3 mb-2"><span class="bg-brand-deep text-white px-3 py-1 rounded text-sm font-bold tracking-widest">HOSE: ${escapeHtml(ctx.ticker)}</span><span class="text-gray-500 font-medium text-sm">Cập nhật: ${f('date')}</span></div><h1 class="text-3xl md:text-4xl font-display font-extrabold text-brand-deep">BÁO CÁO PHÂN TÍCH CHUYÊN SÂU</h1><p class="text-gray-600 mt-1 font-medium">${companyName}</p></div><div class="text-right"><p class="text-xs uppercase tracking-wider text-gray-400 font-bold">Chuyên gia phân tích</p><p class="text-sm font-semibold text-gray-800">Senior Equity Analyst & Geopolitical Strategist</p></div></header>
<main>
...
<div class="card border-2 border-brand-accent/20 bg-blue-50/30"><h2 class="text-2xl font-display font-bold text-brand-deep mb-6 text-center">Chiến lược Giao dịch (Trade Setup)</h2><div class="grid grid-cols-1 md:grid-cols-4 gap-4"><div class="sub-card m-0 text-center"><p class="text-xs font-bold text-gray-400 uppercase mb-2">Vùng Mua</p><p class="text-xl font-bold text-brand-deep">${f('node5.strategy.entry_zone')}</p><p class="text-[10px] text-gray-500 mt-1">${allocation.note}</p></div><div class="sub-card m-0 text-center border-red-100"><p class="text-xs font-bold text-red-400 uppercase mb-2">Cắt Lỗ (SL)</p><p class="text-xl font-bold text-red-600">${f('node5.trading_stop.price')}</p><p class="text-[10px] text-gray-500 mt-1">${f('node5.trading_stop.basis')}</p></div><div class="sub-card m-0 text-center border-green-100"><p class="text-xs font-bold text-green-400 uppercase mb-2">Mục tiêu 1</p><p class="text-xl font-bold text-green-700">${strategyValue(n5.strategy?.tp1)}</p><p class="text-[10px] text-gray-500 mt-1">${strategyRationale(n5.strategy?.tp1)}</p></div><div class="sub-card m-0 text-center border-green-100"><p class="text-xs font-bold text-green-400 uppercase mb-2">Mục tiêu 2</p><p class="text-xl font-bold text-green-700">${strategyValue(n5.strategy?.tp2)}</p><p class="text-[10px] text-gray-500 mt-1">${strategyRationale(n5.strategy?.tp2)}</p></div></div><div class="mt-6 bg-white p-4 rounded-xl border border-brand-accent/10"><h3 class="text-sm font-bold uppercase text-brand-deep mb-3">Lộ trình giải ngân</h3><div class="flex flex-col md:flex-row justify-between gap-4 text-sm"><div class="flex-1"><span class="font-bold text-brand-accent">Bước 1:</span> ${escapeHtml(allocation.steps[0])}</div><div class="flex-1"><span class="font-bold text-brand-accent">Bước 2:</span> ${escapeHtml(allocation.steps[1])}</div><div class="flex-1"><span class="font-bold text-brand-accent">Bước 3:</span> ${escapeHtml(allocation.steps[2])}</div></div></div><div class="mt-4 bg-white p-4 rounded-xl border border-brand-accent/10"><h3 class="text-sm font-bold uppercase text-brand-deep mb-3">Quản trị vị thế (Position Sizing)</h3><div class="flex flex-col md:flex-row justify-between gap-4 text-sm"><div class="flex-1"><span class="font-bold text-brand-accent">Rủi ro/lệnh:</span> ${f('node5.strategy.risk_per_trade_pct_nav')}</div><div class="flex-1"><span class="font-bold text-brand-accent">Tỷ trọng tối đa:</span> ${f('node5.strategy.max_portfolio_weight_pct')}</div><div class="flex-1"><span class="font-bold text-brand-accent">Loại vị thế:</span> ${positionLabel(raw('node5.strategy.position_type'))}</div></div></div></div>
  ...
  function signal(key) {
    return signalDot(n5.conflict_detector?.[key]);
  }
}

function signalDot(value) {
  const signal = String(value ?? '').trim().toLowerCase();
  if (!signal || signal === String(MISSING_SHORT).toLowerCase()) return dotHtml('bg-gray-300', 'Chưa có dữ liệu');
  if (/green|positive|bull|buy|tích cực|tot|tốt|🟢/.test(signal)) return dotHtml('bg-green-600', 'Tích cực');
  if (/red|negative|bear|sell|tiêu cực|xấu|🔴/.test(signal)) return dotHtml('bg-red-600', 'Tiêu cực');
  if (/yellow|neutral|mixed|watch|trung lập|🟡/.test(signal)) return dotHtml('bg-yellow-500', 'Trung lập');
  return dotHtml('bg-gray-300', escapeHtml(formatValue(value)));
}

function dotHtml(colorClass, title) {
  return `<span class="inline-block w-3 h-3 rounded-full ${colorClass}" title="${escapeHtml(title)}"></span>`;
}
...
function renderScreeningSnapshot(n1, n5) {
  ...
  return `<div class="card border-2 border-purple-200 mb-6"><h2 class="text-xl font-display font-bold text-brand-deep mb-1">Tóm tắt sàng lọc</h2><p class="text-xs text-gray-400 mb-4">Nguồn: StockScreener — đây là bối cảnh sàng lọc ban đầu, KHÔNG phải điểm số của CRSM</p><div class="grid grid-cols-3 md:grid-cols-4 gap-3 mb-4">${metric('Điểm',s.screen_score)}${metric('Hạng',s.screen_rank)}${metric('Phân loại',s.screen_grade)}${metric('Chất lượng',s.quality_score)}${metric('Tăng trưởng',s.growth_score)}${metric('Định giá',s.valuation_score)}${metric('Động lượng',s.momentum_score)}${metric('Định giá sai',s.mispricing_score ?? s.opportunity_score)}</div><div class="flex flex-col md:flex-row justify-between items-center gap-3 bg-white p-4 rounded-xl border border-brand-accent/10"><div class="text-center md:text-left"><span class="text-xs text-gray-400 uppercase block">Điểm CRSM</span><strong class="text-2xl text-brand-deep">${escapeHtml(formatValue(n5.ai_score?.value))}/100</strong></div><div class="text-center"><span class="badge bg-purple-100 text-purple-700">Sàng lọc → CRSM: ${escapeHtml(statusLabel(n5.screen_vs_crsm?.status))}</span><p class="text-[10px] text-gray-500 mt-1">${escapeHtml(formatValue(n5.screen_vs_crsm?.interpretation))}</p></div></div></div>`;
}

function buildNode6ASystemInstruction() {
  return `${node6aPrompt}
...
- Output exactly one complete HTML document, starting with <!DOCTYPE html> or <html and ending with </html>.
- Preserve the visual report structure and professional institutional style. Do not produce a plain text page.
- Prefer concise, action-oriented card copy. Use all available Node 1-5 data, especially scenarios, trade setup, allocation plan, risk notes, source notes, and screening snapshot when mode is SCREENED.
- Missing data must be shown as "Chưa có dữ liệu" or "Data not available" consistently, never as raw null, undefined, N/A, or blank boxes.
- Do not invent numbers. If a value is inferred from upstream text, make the uncertainty visible.`;
}

function buildNode6APrompt(ctx) {
  return `Generate the final CRSM HTML visual report from this combined analysis JSON.
...
function cleanHtml(value) {
  ...
  if (start >= 0 && end >= start) text = text.slice(start, end + 7).trim();
  return text;
}

function isUsableHtml(value) {
  const html = String(value || '').trim().toLowerCase();
  if (!html.includes('<html') || !html.includes('</html>')) return false;
  if (!html.includes('báo cáo') && !html.includes('bao cao')) return false;
...
function scenarioTarget(obj) {
  return escapeHtml(formatValue(obj?.target ?? obj?.target_price ?? obj?.price_target ?? obj?.bear_price ?? obj?.stop_loss ?? obj?.price));
}
...
function positionLabel(value) {
  ...
  if (v === 'INITIAL') return 'Vị thế ban đầu';
  if (v === 'ADD-ON' || v === 'ADD_ON') return 'Gia tăng vị thế';
  return escapeHtml(formatValue(value));
}
...
function percentText(value) {
  ...
  return /%$/.test(text.trim()) ? escapeHtml(text) : `${escapeHtml(text)}%`;
}
